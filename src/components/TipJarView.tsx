...
export default function TipJarView() {
  ...
  return (
    <div className="max-w-lg mx-auto space-y-4">
      <div className="bg-gradient-to-r from-amber-900/40 via-orange-900/30 to-rose-900/40 border border-[#2A2A36] rounded-2xl p-5">
      ...
      <div className="bg-[#16161D] border border-[#2A2A36] rounded-xl p-5 space-y-4">
        <div>
          <label className="text-xs font-semibold text-[#8888A0] uppercase tracking-wider mb-2 block">
            Which bar are you at?
          </label>
          <div className="relative">
            <button
              onClick={() => setBarPickerOpen(!barPickerOpen)}
              className={`w-full px-4 py-3 bg-[#1E1E28] border rounded-xl text-sm text-left flex items-center justify-between transition-colors ${
                barPickerOpen ? "border-purple-500/50" : "border-[#2A2A36]"
              }`}
            >
              {selectedBar ? (
                ...
                <span className="text-[#8888A0]">Select your bar...</span>
              )}
              <ChevronDown className={`w-4 h-4 text-[#8888A0] shrink-0 transition-transform ${barPickerOpen ? "rotate-180" : ""}`} />
            </button>
            ...
            <AnimatePresence>
              {barPickerOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -4 }}
                  className="absolute z-20 top-full mt-2 left-0 right-0 bg-[#1E1E28] border border-[#2A2A36] rounded-xl overflow-hidden shadow-xl shadow-black/40"
                >
                  <div className="p-2 border-b border-[#2A2A36]">
                    <div className="relative">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#8888A0]" />
                      <input
                        type="text"
                        placeholder="Search bars..."
                        value={barSearch}
                        onChange={(e) => setBarSearch(e.target.value)}
                        className="w-full pl-9 pr-3 py-2 bg-[#16161D] border border-[#2A2A36] rounded-lg text-xs placeholder-[#8888A0] focus:outline-none focus:border-purple-500/50"
                        autoFocus
                      />
                    </div>
                  </div>
                  <div className="max-h-60 overflow-y-auto">
                    {filteredBars.length === 0 ? (
                      <div className="px-4 py-6 text-center text-xs text-[#8888A0]">No bars found</div>
                    ) : (
                      filteredBars.map((bar) => (
                        <button
                          ...
                          className={`w-full px-4 py-3 text-left hover:bg-[#16161D] transition-colors flex items-center gap-3 ${
                            selectedBar?.id === bar.id ? "bg-purple-500/10" : ""
                          }`}
                        >
                          <MapPin className="w-3.5 h-3.5 text-purple-400 shrink-0" />
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-medium truncate">{bar.name}</div>
                            <div className="text-[10px] text-[#8888A0] truncate">{bar.address}</div>
                          </div>
        ...
        <div>
          <label className="text-xs font-semibold text-[#8888A0] uppercase tracking-wider mb-2 block">Your Name</label>
          <input
            type="text"
        ...
        <div>
          <label className="text-xs font-semibold text-[#8888A0] uppercase tracking-wider mb-2 block">Song Request</label>
          <input
            type="text"
        ...
        <div>
          <label className="text-xs font-semibold text-[#8888A0] uppercase tracking-wider mb-2 block">Tip Amount</label>
      ...
      {selectedBar && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="bg-[#16161D] border border-[#2A2A36] rounded-xl p-4 flex items-center gap-3"
        >
          <MapPin className="w-4 h-4 text-purple-400 shrink-0" />
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium">{selectedBar.name}</div>
            <div className="text-xs text-[#8888A0] truncate">{selectedBar.address}</div>
          </div>
